...
var emitSQLTool = map[string]any{
	...
	"input_schema": map[string]any{
		...
		"properties": map[string]any{
			"answerable": map[string]any{"type": "boolean", "description": "false if the input is a greeting, chit-chat, or not a question about the factory data (then leave sql empty)."},
...
func emitSQL(ctx context.Context, question, schema string, prev *prevTurn) (string, error) {
...
func AskData(c *fiber.Ctx) error {
	...
	rawSQL, err := emitSQL(ctx, strings.TrimSpace(body.Question), buildSchemaContext(ctx, user.OrgId), prev)
	if errors.Is(err, errNotDataQuestion) {
		return c.Status(422).JSON(fiber.Map{"success": false, "error": fiber.Map{"message": err.Error()}})
	}
	if err != nil {
	...
	option, err := emitEChart(ctx, body.Question, cols, sampleRows(rows, 20))
	if err != nil {
