...
const rangeDurationMs = computed(() =>
  new Date(endDateTime.value).getTime()
  - new Date(startDateTime.value).getTime(),
);
...
const hasProblems = computed(() => {
  if (lowerLimit.value === null && upperLimit.value === null) {
    return false;
  }

  return mergedData.value.some(p => {
    if (upperLimit.value !== null && p.value > upperLimit.value) {
      return true;
    }

    if (lowerLimit.value !== null && p.value < lowerLimit.value) {
      return true;
    }
...
function handleZoom(e: any) {
  ...
  // IMPORTANT:
  // Directly update chart WITHOUT recreating whole option
  chartRef.value?.chart?.setOption({
    yAxis: {
      min: visibleYMin.value,
...
const shouldRotate = computed(() =>
  rangeDurationMs.value >= 7 * 24 * 60 * 60 * 1000,
);
...
const option = computed<EChartsOption>(() => {
  ...
  return {
    ...
    grid: {
      ...
      right: 60,
      top: 16,
      bottom: shouldRotate.value ? 52 : 30,
      containLabel: false,
    },
    ...
    tooltip: {
      ...
      formatter: (params: any) => {
        ...
        const thr =
          threshold.value !== null
            ? `<br/><span style="color:#6366f1">◎ target: ${threshold.value}</span>`
            : '';

        const up =
          upperLimit.value !== null
            ? `<br/><span style="color:#f59e0b">↑ upper: ${upperLimit.value}</span>`
            : '';

        const lo =
          lowerLimit.value !== null
            ? `<br/><span style="color:#f59e0b">↓ lower: ${lowerLimit.value}</span>`
            : '';

        return `
          <div style="font-family:monospace;line-height:1.6">
            <span style="color:#9ca3af;font-size:11px">
              ${formatTooltipTime(pt?.ts ?? '')}
            </span>

            <span style="color:#6b7280;font-size:10px">
              · avg/${bucketLabel.value}
            </span>
    ...
    <template v-else>
      ...
      <div class="flex items-center justify-between px-1 pt-0.5 flex-shrink-0 gap-1">
        ...
        <div class="flex items-center gap-1.5 ml-auto">
          <span class="text-[9px] text-gray-600 hidden sm:block">
